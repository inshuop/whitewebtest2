  ...
  if (req.method === "POST") {
    ...
    return uploadMiddleware(req, res, async () => {
      try {
        ...
        const picture = `/uploads/article/${uploadedFile.filename}`;
        ...
        return res.status(200).json(article); // Respond with the created article object
      } catch (error) {
    ...
    try {
      ...
      const deletedArticle = await prisma.article.delete({
        where: {
          id: parseInt(articleId as string, 10), // Ensure it's treated as a number
        },
      });
