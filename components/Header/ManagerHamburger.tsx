...
import { FaFacebook, FaInstagram, FaLinkedin, FaHome } from "react-icons/fa";
...
const HomeHamburger: React.FC<DivisionHamburgerProps> = ({ setHamburgerMenuOpen }) => {
  const [openSubmenus, setOpenSubmenus] = useState<{ [key: string]: boolean }>({});
  const [isHidden, setIsHidden] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);

  const options = [
    { title: "Careers", path: "/careers", icon: FaHome },
    // { title: "Option 2", path: "/option2", icon: FaInfoCircle },
    // { title: "Option 3", path: "/option3", icon: FaServicestack },
  ];
      ...
      >
        <div>
          ...
          <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-bold textblue dark:text-gray-300">
              White Aluminium
            </h2>
            ...
            {options.map((option, index) => (
              <React.Fragment key={index}>
                <li className="group flex items-center space-x-2">
                  {React.createElement(option.icon, { className: "textblue dark:text-gray-300" })}
                  <Link
                    href={option.path}
                    className="block text-lg font-medium textblue dark:text-gray-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors"
                  >
                    {option.title}
                  </Link>
                </li>
                    ...
                    <>
                      <div className="flex justify-between items-center">
                        ...
                        >
                          ...
                          <span className="block text-lg font-medium textblue dark:text-gray-300">
                            {menuItem.title}
                          </span>
                        </div>
                        <button
